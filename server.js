...
wss.on("connection", (ws) => {
  ...
  if (
    !currentSessionId ||
    !sessions.has(currentSessionId) ||
    (sessions.get(currentSessionId).mapClient &&
      sessions.get(currentSessionId).panelClient)
  ) {
  ...
  ws.on("message", (message) => {
    try {
      ...
      if (session.mapClient && session.panelClient) {
        if (data.clientType === "map") {
          if (session.panelClient.readyState === WebSocket.OPEN) {
            console.log(
              `Reenviando mensaje del map al panel en sesión ${sessionId}`,
            );
            session.panelClient.send(JSON.stringify(data));
            console.log(`Mensaje reenviado al panel:`, data);
          } else {
            console.warn(
              `Panel en sesión ${sessionId} no está listo para recibir mensajes`,
            );
          }
        } else if (data.clientType === "panel") {
          if (session.mapClient.readyState === WebSocket.OPEN) {
            console.log(
              `Reenviando mensaje del panel al map en sesión ${sessionId}`,
            );
            session.mapClient.send(JSON.stringify(data));
            console.log(`Mensaje reenviado al map:`, data);
          } else {
            console.warn(
              `Map en sesión ${sessionId} no está listo para recibir mensajes`,
            );
          }
        }
      } else {
        console.log(
          `No se puede reenviar mensaje en sesión ${sessionId}: uno de los clientes no está conectado.`,
        );
      }
      ...
      if (session.mapClient && session.panelClient && !session.paired) {
        console.log(
          `Session ${sessionId} is now paired with a map and panel client`,
        );
        session.paired = true;

        const pairMessageToMap = {
          message: "Paired with panel client",
          sessionId,
        };
        const pairMessageToPanel = {
          message: "Paired with map client",
          sessionId,
        };
        ...
        if (session.mapClient.readyState === WebSocket.OPEN) {
          ...
          console.log(
            `Sent to map client [sessionId: ${sessionId}]:`,
            pairMessageToMap,
          );
        }
        if (session.panelClient.readyState === WebSocket.OPEN) {
          ...
          console.log(
            `Sent to panel client [sessionId: ${sessionId}]:`,
            pairMessageToPanel,
          );
        }
  ...
  console.log(
    `Sent to client [sessionId: ${currentSessionId}]:`,
    welcomeMessage,
  );
  ...
  ws.on("close", () => {
    ...
    if (session) {
      ...
      if (session.panelClient === ws) {
        ...
        console.log(
          `Panel client disconnected from session ${currentSessionId}`,
        );
      }
      ...
      if (!session.mapClient && !session.panelClient) {
        ...
        console.log(
          `Session ${currentSessionId} has been deleted after both clients disconnected`,
        );
      }
    }
  });
